...
function refresh(){selected=read();if(!selected)return;
 ...
 const status=m.revised?'Bathroom batch 1 applied to this example. Other rooms and bathroom operating clearances remain under review.':'Earlier planning draft. The revised bathroom batch has not yet been applied to this option.';
 ...
 root.querySelectorAll('.planning-pdf').forEach(a=>{a.href='planning/pdfs/'+m.id+'.pdf';a.download=m.id+'.pdf';});
